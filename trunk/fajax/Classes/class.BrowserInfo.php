	...
	public function BrowserInfo(){
	  ...
	  // test for Mozilla Alpha/Beta Versions
		...
		// test for Safari
			...
			$bd['browser'] = "Safari";
			$sVer = "";
			$verPos = stripos($agent, "version");
			$namePos = stripos($agent, "safari");
			$techPos = stripos($agent, "mobile");
			if((int)$techPos > 0) {
    			$sVer = substr($agent, $verPos + 7);
    			$techPos = stripos($sVer, "mobile");
    			$sVer = substr($sVer, 0, $techPos);
    			$sVer = trim($sVer);
			}
			else {
    			try {
        			if($namePos > $verPos) {
            			$sVer = substr($agent, $verPos + 7);
            			$namePos = stripos($sVer, "safari");
            			$sVer = substr($sVer, 1, $namePos - 1);
            			$sVer = trim($sVer);
        			}
        			else {
            			$sVer = substr($agent, $namePos + 8);
            			$sVer = trim($sVer);
                    }
    			}
    			catch(Exception $e) {
